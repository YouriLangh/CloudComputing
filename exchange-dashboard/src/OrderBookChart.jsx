...
const OrderBookChart = ({ orderBookData }) => {
  ...
  const prepareChartData = (symbol) => {
    ...
    const bidPrices = sortedBids.map(bid => bid.price);
    const bidQuantities = sortedBids.map(bid => bid.quantity);
    const askPrices = sortedAsks.map(ask => ask.price);
    const askQuantities = sortedAsks.map(ask => ask.quantity);
